...
(defn add-apple
  ...
  (let [possible-positions (vec
                             (-> all-grid-coordinates
                                 (s/difference (:apples state))
                                 (s/difference (:wall state))
                                 (s/difference (set (:snake state)))))
...
(defn setup []
  ; (js/alert "Available fonts:" (q/available-fonts))
  (q/frame-rate 10)
  (q/stroke-weight 2)
...
(defn draw [state]
  ...
  (q/fill 0xffbc5148)
  (q/stroke 0xffbc5148)
  (q/text (str "SCORE: " (:score state)) 15 0 200 15)
  (q/text (str "HIGH SCORE: " @high-score) 15 (- height 10) 200 height)
...
(defn update-apple-state
  ...
  Any apples lying within the new wall get removed
  ...
  (if (contains? (:apples state) snake-head)
    (let [new-state (-> state
      ...
      (if (zero? (mod (:score new-state) 3))
        ; We need to ensure any apples placed where the new wall is disappear
        (let [new-wall (generate-wall)]
          (-> (assoc new-state :wall new-wall)
              (assoc :apples (s/difference (:apples new-state) new-wall))))
        new-state))
    state))
