(ns drawing.circle.fish-scale-pattern
...
; Change background-colour to black for a different look
(def background-colour white)
...
; Use this function instead of the above for a random colour selection
; (defn
;   get-next-colour
;   "Random selection of colour"
;   [last-colour]
;   (rand-nth (keys colours)))
...
(defn
  draw-state
  [state]
  (q/background background-colour)
  (doseq
    [[x y c] (:circles state)]
    (let
  ...
  (let [
    ...
    (if (>= new-y (+ sHeight diameter))
      (let [pause (:pause state)]
        (if (> pause 0)
          ; If we are paused, return state with pause decremented by 1
          (update-in state [:pause] dec)
          ; Otherwise, reset and carry over the colour
          (assoc-in (setup-state) [:last-circle 2] new-colour)))
      
      
      #_ "Otherwise return an updated version of the state"
      {:circles     new-circles
...
(defn -main
  []
  (q/sketch
    :title  "Fish Scale Pattern"
    :host   "fish-scale-pattern"
    :size   [sWidth sHeight]
    :setup  setup
    :update update-state
    :draw   draw-state
