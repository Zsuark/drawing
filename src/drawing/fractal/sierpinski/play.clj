(ns drawing.fractal.sierpinski.play
...
(defn
  ...
  []
  ; {:instruction-list (list :A :+ :+ :+ :A)
  ; {:instruction-list (list :A :+ :+ :+ :+ :A)
  {:instruction-list (list :A :+ :+ :+ :+ :A :+ :+ :+ :+ :A)
  ; {:instruction-list (list :A :+ :+ :+ :+ :A :+ :+ :+ :+ :A :+ :+ :+ :+ :A)
   :iteration    0
   :unit-length  (- (/ sWidth 2) 10)
   :pause        5})
...
(defn
  ...
  [state]
  ; (println "In draw-state, time is:" (nowString))
  (q/background 255)
  ...
  ; (println "current fps:" (q/current-frame-rate))
